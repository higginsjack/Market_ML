...
def instancesOfGrowth(file_path):
    """
    Testing to see what percentage gains are normal
    """
    gdf = pd.read_csv(file_path)
    print("Max 900:", max(gdf["900"]), "Timestamp", max(range(len())))
    print("Max 1800:", max(gdf["1800"]), "Timestamp", max(range(len())))
    print("Max 3600:", max(gdf["3600"]), "Timestamp", max(range(len())))
    print("Max 7200:", max(gdf["7200"]), "Timestamp", max(range(len())))
    print("Max 10800:", max(gdf["10800"]), "Timestamp", max(range(len())))
    print("- - - - - - - - - - - - - - - - - - - - - - - - - - -")
    for i in [0.01, 0.005, ]
    print("900   | 0.01 instances",  len(gdf[gdf["900"] > i]))
    print("1800  | 0.01 instances",  len(gdf[gdf["1800"] > i]))
    print("3600  | 0.01 instances",  len(gdf[gdf["3600"] > i]))
    print("7200  | 0.01 instances",  len(gdf[gdf["7200"] > i]))
    print("10800 | 0.01 instances",  len(gdf[gdf["10800"] > i]))
...
instancesOfGrowth("data/AMZN/aggregate/AMZN_growth.csv")